<x-layouts.layout-dash>
    <main x-data="{
        ...
        resetAndOpen(modal) {
            this.currentCustomer = { id: null, name: '', email: '', phone: '' };
        ...
        <header class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 lg:mb-12 gap-6">
            ...
            <div class="flex items-center justify-between w-full md:w-auto gap-6">
                ...
                <button @click="resetAndOpen('customer-create')"
            ...
            <div class="overflow-y-auto max-h-[calc(100vh-320px)] custom-scrollbar">
                <table class="w-full text-left border-collapse">
                    <thead class="sticky top-0 bg-zinc-50 border-b border-zinc-100 z-10">
                        <tr>
                            <th class="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-zinc-400">Identificação</th>
                            <th class="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-zinc-400">E-mail</th>
                            <th class="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-zinc-400">Telefone</th>
                            ...
                            <tr class="hover:bg-zinc-50/50 transition-colors group">
                                <td class="px-8 py-5">
                                    <div class="flex items-center gap-4">
                                        <div class="w-9 h-9 rounded-2xl bg-zinc-950 flex items-center justify-center text-[11px] font-black text-white shadow-lg shadow-zinc-200">
                                            {{ substr($c->name, 0, 2) }}
                                        </div>
                                        <span class="text-sm font-black text-zinc-900 tracking-tight">{{ $c->name }}</span>
                                    </div>
                                </td>
                                <td class="px-8 py-5 text-sm text-zinc-500 font-bold italic">{{ $c->email }}</td>
                                <td class="px-8 py-5 text-sm text-zinc-900 font-black tracking-tighter">{{ $c->phone_formatted }}</td>
                                <td class="px-8 py-5 text-right">
                                    <div class="flex justify-end gap-6 items-center">
                                        ...
                                        <form class="mb-1" action="{{ route('customers.destroy', $c) }}" method="POST">
                                            @csrf @method('DELETE')
                                            <button class="text-[10px] font-black uppercase text-zinc-300 hover:text-red-600 transition-colors">Excluir</button>
                                        </form>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="4" class="px-8 py-12 text-center">
                                    <p class="py-20 text-zinc-400 font-bold">Nenhum cliente cadastrado</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
        ...
        <section class="lg:hidden space-y-4 pb-24">
            @forelse ($customers as $c)
                <div class="p-6 bg-white border border-zinc-100 rounded-[28px] shadow-sm flex flex-col gap-5">
                    <div class="flex items-center gap-4">
                        <div class="w-12 h-12 rounded-[18px] bg-zinc-950 flex items-center justify-center text-xs font-black text-white shadow-xl shadow-zinc-200">
                            {{ substr($c->name, 0, 2) }}
                        </div>
                        <div class="flex-1">
                            <h3 class="text-base font-black text-zinc-900 leading-tight tracking-tight">{{ $c->name }}</h3>
                            <p class="text-[10px] font-black text-zinc-400 uppercase tracking-widest mt-0.5">Cliente Ativo</p>
                        </div>
                    </div>
                    ...
                    <div class="space-y-2 pt-4 border-t border-zinc-50">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <span class="text-xs font-black text-zinc-900 tracking-tighter">{{ $c->phone_formatted }}</span>
                        </div>
                    </div>
                    ...
                    <div class="flex gap-3 items-center">
                        <button
                            @click="currentCustomer = { id: {{ $c->id }}, name: '{{ addslashes($c->name) }}', email: '{{ $c->email }}', phone: '{{ $c->phone }}' }; activeModal = 'customer-edit';"
                            class="flex-1 bg-zinc-50 h-11 rounded-xl text-[10px] font-black uppercase tracking-widest text-zinc-600">
                            Editar
                        </button>
                        <form action="{{ route('customers.destroy', $c) }}" method="POST" class="shrink">
                            @csrf @method('DELETE')
                            <button class="w-11 h-11 flex items-center justify-center bg-rose-50 text-rose-600 rounded-xl">
            ...
            @empty
                <div class="py-20 text-center text-zinc-300 font-black uppercase text-[10px] tracking-widest">Nenhum cliente cadastrado</div>
            @endforelse
        </section>
            ...
            <div class="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 backdrop-blur-sm p-0 sm:p-4"
                ...
                <div class="bg-white w-full max-w-lg rounded-t-[32px] sm:rounded-[40px] shadow-2xl overflow-hidden border border-zinc-100"
                    ...
                    <form class="p-8 space-y-5"
                        :action="activeModal === 'customer-create' ? '{{ route('customers.store') }}' : `/dashboard/customers/${currentCustomer.id}`"
                        method="POST">
                        @csrf
                        ...
                        <div class="space-y-1.5">
                            <label class="text-[10px] font-black uppercase tracking-widest text-zinc-400 ml-1">Nome Completo</label>
                            <input type="text" name="name" x-model="currentCustomer.name" required
                                class="w-full h-12 px-5 rounded-2xl border border-zinc-700 bg-zinc-50 focus:bg-white focus:border-black transition-all text-sm font-black outline-none shadow-sm">
                        </div>

                        <div class="space-y-1.5">
                            <label class="text-[10px] font-black uppercase tracking-widest text-zinc-400 ml-1">E-mail Principal</label>
                            <input type="email" name="email" x-model="currentCustomer.email" required
                                class="w-full h-12 px-5 rounded-2xl border border-zinc-700 bg-zinc-50 focus:bg-white focus:border-black transition-all text-sm font-black outline-none shadow-sm">
                        </div>

                        <div class="space-y-1.5">
                            <label class="text-[10px] font-black uppercase tracking-widest text-zinc-400 ml-1">Telefone / WhatsApp</label>
                        ...
                        <div class="pt-6 flex gap-3">
                            ...
                            <button type="submit"
                                class="flex-1 h-14 bg-black text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-zinc-800 shadow-xl transition-all">
                                Salvar Registro
                            </button>
                        </div>
                    </form>
